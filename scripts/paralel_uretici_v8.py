#!/usr/bin/env python3
"""
PARALEL SKILL ÜRETİCİ v8 - SÜPER HIZLI (FIXED: Ollama concurrency, naming)
Aynı anda 3 video işler, 3 farklı API kullanır
Sırayla dene: OpenRouter GPT-4o-mini (öncelikli), Gemini, Ollama qwen2.5-coder
Ollama'ya aynı anda sadece 1 istek (diğer thread'ler beklemez)
"""
...
lock = Lock()
# Ollama can only handle 1 request at a time
ollama_sem = Semaphore(1)
...
def extract_name(content):
    """Extract a meaningful name from the SKILL.md content, avoid 'hermes-agent' default"""
    m = re.search(r'^name:\s*(.+)$', content, re.MULTILINE)
    if m:
        n = m.group(1).strip().strip('"\'')
        n2 = re.sub(r'[^a-z0-9-]', '', n.lower())
        # Skip generic names
...
def process_one_video(video_info):
    ...
    # 1. OpenRouter (öncelikli - en hızlı, en kaliteli)
    if OPENROUTER_API_KEY and not content:
        try:
            prompt = f"""Create a Hermes Agent skill from this cybersecurity video transcript.
Output YAML frontmatter with:
- name: a SHORT unique kebab-case name (2-4 words, describing the technique/topic, e.g. 'dns-exfiltration' or 'bloodhound-analysis')
...
Then the body with a concise step-by-step guide.
...
{text[:10000]}"""
    ...
    if GOOGLE_API_KEY and not content:
        try:
            prompt2 = f"""Create a Hermes Agent skill from this cybersecurity transcript.
Output YAML frontmatter with name (short kebab-case), description, version=1.0, category=security, source=https://youtu.be/{vid_id}, tags=[relevant].
Then body with step-by-step guide.

Transcript:
{text[:10000]}"""
            url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={GOOGLE_API_KEY}"
            r = requests.post(url,
                json={"contents":[{"parts":[{"text":prompt2}]}],"generationConfig":{"maxOutputTokens":2000,"temperature":0.3}},
                timeout=45)
            if r.status_code == 200:
    ...
    # 3. Ollama (en yavaş - önce kontrol et)
    if not content:
        try:
            ...
            try:
                ...
                log(f"  ⚠️ Ollama {vid_id}: server unreachable, skipping")
                raise Exception("skip")  # Go to fallback
            
            with ollama_sem:
                prompt3 = f"""Create a Hermes Agent skill from this cybersecurity transcript.
YAML frontmatter with name=kebab-case(short), description, version=1.0, category=security, source=https://youtu.be/{vid_id}, tags=[relevant].
Body=guide format.
...
# === MAIN LOOP ===
log("=" * 50)
log("PARALEL SKILL ÜRETİCİ v8 BAŞLADI")
log(f"API: Gemini={'✅' if GOOGLE_API_KEY else '❌'} OpenRouter={'✅' if OPENROUTER_API_KEY else '❌'} Ollama=✅ (1-thread)")
...
baslangic = time.time()
success_count = 0
source_counts = {"gemini": 0, "openrouter": 0, "ollama": 0, "cached": 0, "fallback": 0}

for batch_start in range(0, min(len(pending), 120), MAX_WORKERS):
...
sc = source_counts
log(f"   Kaynaklar: OpenRouter={sc['openrouter']} Gemini={sc['gemini']} Ollama={sc['ollama']} Önbellek={sc['cached']} Fallback={sc['fallback']}")
log(f"🧠 Önbellek: {len(cache)} kayıt")
