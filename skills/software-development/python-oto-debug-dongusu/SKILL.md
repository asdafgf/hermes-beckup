...
## Qwen Entegrasyonu (Ollama REST API)

Qwen2.5-coder ile çalışırken asla `ollama run` kullanma — PTY gerektirir, Windows'ta background'da çalışmaz. Her zaman curl REST API kullan:

```bash
# DOĞRU
curl -s --max-time 120 -X POST http://localhost:11434/api/generate \
  -d "$(python -c 'import json; print(json.dumps({"model":"qwen2.5-coder:7b","prompt":"...","stream":False}))')"
```

### qwen_sor() Şablon Fonksiyonu

```python
import urllib.request, json

def qwen_sor(prompt, model="qwen2.5-coder:7b", timeout=120):
...
### ÖNEMLİ: Python JSON payload'da `False` vs `false`
`json.dumps({'stream':False})` yaz (büyük F). `{'stream':false}` yazarsan `NameError: name 'false' is not defined` hatası.

---

## Script Hazırlama Kuralları (Bash)

- **Parantez kullanma.** `echo` ve `# yorum` satırlarında parantez `()` kullanma — bash alt-shell olarak yorumlar ve syntax hatası verir.
- `echo "=== KONU 1 ==="` ✅ — `echo "=== KONU 1 (BASLIK) ==="` ❌
- `sed` ile temizlik yaparken fonksiyon isimlerindeki parantezleri koru: `run_safe() {` değişmemeli.
- `/tmp/` Windows'ta yok. Geçici dosyalar için `C:\Users\<user>\AppData\Local\Temp\` kullan.
