...
# Reference: scheduled-agent-learning skill
# 
# Bu script ~/.hermes/scripts/python_ogrenim.sh olarak calistirilir
# Cronjob: hermes cronjob create python-ogrenim-saatlik --schedule "0 * * * *" --script python_ogrenim.sh --deliver origin --repeat 24
...
# Lock kontrolu - ayni anda iki kez calismasin
if [ -f "$LOCK_FILE" ]; then
...
if [ $SIRKAT -gt $TOP_KONU ]; then
    SIRKAT=1  # Basa don - loop mode
fi

KONU_ADI="Konu $SIRKAT/$TOP_KONU"
KONU_ICERIK="${KONULAR[$((SIRKAT-1))]}"

# Qwen'e sor - curl REST API (ollama run kullanma!)
qwen_sor() {
    local prompt="$1"
    local payload
...
# Progress guncelle
echo "{\"son_konu\":$SIRKAT,\"toplam\":$TOP_KONU,\"tarih\":\"$TARIH\",\"konu\":\"${KONU_ICERIK:0:50}\"}" > "$RAPOR_DIZINI/progress.json"
